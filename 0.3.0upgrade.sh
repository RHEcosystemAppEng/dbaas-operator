...
upgradeInstanceCRs () {
   echo ""
   echo "Upgrading RHODA 0.2.0 dbaasinstance CR(s)"
...
if [ ! -z "${installns02}" ] && [ ! -z "${installns03}" ]; then
    echo "Verifying '${ocuser}' permissions in the ${installns02} project"
    ...
    INS_CRDS="dbaasinstance crdbdbaasinstance crunchybridgeinstance mongodbatlasinstance rdsinstance"
    for insCRD in $INS_CRDS; do
...
EOF
        ...
        if [ ! -z "${addonsApi}" ]; then
            if [ $(oc auth can-i get addons --all-namespaces) == "yes" ]; then
                ...
                # if sub has been recreated by addon, proceed w/ secondary cleanup
