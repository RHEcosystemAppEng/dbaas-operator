...
| *`policy`* __xref:{anchor_prefix}-github-com-rhecosystemappeng-dbaas-operator-api-v1beta1-dbaaspolicyspec[$$DBaaSPolicySpec$$]__ | The policy for this inventory. Completely overrides any existing DBaaSPolicy settings in this namespace. This change may break v1alpha1 inventories where only `disableProvisions` was set, but the broader DBaaSPolicy allowed certain namespaces.
|===
